...
void main() {
  test('queues offline work and completes it after reconnecting', () async {
    ...
    when(
      () => remote.create('Survey', any(), any(), any()),
    ).thenAnswer((_) async => const SyncResult.success());
    ...
    await surveys.createSurvey(SyncFixtures.survey());
    ...
    final completed = PendingOperationsDao(database)
        .watchAllOperations()
        .firstWhere(
          (items) =>
    ...
    verify(() => remote.create('Survey', any(), any(), any())).called(1);
